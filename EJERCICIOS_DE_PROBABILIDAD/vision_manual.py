# -*- coding: utf-8 -*-
# Visión por Computador - Versión Manual Sin Librerías

# Datos del problema
precision = 0.92                   # Precisión del 92%
P_F = 0.40                        # 40% femeninas  
P_M = 0.60                        # 60% masculinas

# Calcular P(error|F)
P_error_F = 1 - precision

print("VISIÓN POR COMPUTADOR - VERSIÓN MANUAL")
print("="*38)
print("\nDatos:")
print(f"Precisión = {precision} = 23/25")
print(f"P(Femenina) = {P_F} = 2/5")
print(f"P(Masculina) = {P_M} = 3/5")

print("\nCálculo:")
print("P(Error|F) = 1 - Precisión")
print(f"P(Error|F) = 1 - {precision} = {P_error_F}")

print("\nRESPUESTA:")
print(f"P(Error|F) = {P_error_F}")
print("Fracción: 2/25")
print(f"Porcentaje: {P_error_F*100}%")

print(f"\n✅ El modelo se equivoca en {P_error_F*100}% de imágenes femeninas")